...
if (isset($_POST['enregistrer'])) {
    ...
    if (empty($erreurs)) {
        ...
        // Appeler la fonction pour obtenir un numéro de chambre disponible de type "suite"
        $numChambreDisponible = obtenir_numero_chambre_disponible('Suite');
        ...
        if (!empty($numChambreDisponible)) {
            ...
            if (!empty($numClient)) {
                ...
                if ($resultatInsertionReservation) {
                    ...
                    if (!empty($_POST["nom_acc"]) && !empty($_POST["contact_acc"])) {
                        // Enregistrer le premier accompagnateur
                    ...
                    if (!empty($_POST["nom_acc2"]) && !empty($_POST["contact_acc2"])) {
                        // Enregistrer le deuxième accompagnateur
                        if (!vérifier_contact_accompagnateur_exist_in_db($_SESSION['donnees-reservation']["contact_acc2"])) {
                            // Appeler la fonction pour insérer les informations du deuxième accompagnateur dans la table "client"
                            $resultatInsertionAccompagnateur2 = enregistrer_accompagnateur($_SESSION['donnees-reservation']['nom_acc2'], $_SESSION['donnees-reservation']['contact_acc2']);
                            $numAccompagnateur2 = recuperer_num_acc_par_son_contact($_SESSION['donnees-reservation']['contact_acc2']);
                    ...
                    if (!empty($_POST["nom_acc3"]) && !empty($_POST["contact_acc3"])) {
                        // Enregistrer le deuxième accompagnateur
                        if (!vérifier_contact_accompagnateur_exist_in_db($_SESSION['donnees-reservation']["contact_acc3"])) {
                            // Appeler la fonction pour insérer les informations du deuxième accompagnateur dans la table "client"
                            $resultatInsertionAccompagnateur3 = enregistrer_accompagnateur($_SESSION['donnees-reservation']['nom_acc3'], $_SESSION['donnees-reservation']['contact_acc3']);
                            $numAccompagnateur3 = recuperer_num_acc_par_son_contact($_SESSION['donnees-reservation']['contact_acc3']);
                    ...
                    if (!empty($_POST["nom_acc4"]) && !empty($_POST["contact_acc4"])) {
                        // Enregistrer le deuxième accompagnateur
                        if (!vérifier_contact_accompagnateur_exist_in_db($_SESSION['donnees-reservation']["contact_acc4"])) {
                            // Appeler la fonction pour insérer les informations du deuxième accompagnateur dans la table "client"
                            $resultatInsertionAccompagnateur4 = enregistrer_accompagnateur($_SESSION['donnees-reservation']['nom_acc4'], $_SESSION['donnees-reservation']['contact_acc4']);
                            $numAccompagnateur4 = recuperer_num_acc_par_son_contact($_SESSION['donnees-reservation']['contact_acc4']);
            ...
            // Aucune chambre disponible de type "suite"
...
$_SESSION['reservation-suite-message-erreur-global'] = $message_erreur_global;
$_SESSION['reservation-suite-message-success-global'] = $message_success_global;
header('location: ' . PATH_PROJECT . 'administrateur/dashboard/reservation-suit');
